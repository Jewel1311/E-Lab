...
class _BookingDetailsState extends State<BookingDetails> {
  ...
  Future getBookingInfo() async {
    ...
    bookingDetails = await supabase.from('booking').select().match({'id': bookingId['bookingId']});
    labDetails = await supabase.from('labs').select().match({'id': bookingDetails[0]['lab_id']});
    patientDetails = await supabase.from('patient').select().match({'id':bookingDetails[0]['patient_id']});
            ...
            Padding(
              ...
              child: Column(
                ...
                Row(
                  children: [
                    const Text("status: "),
                    Text(bookingDetails[0]['status'], style: TextStyle(color: Colors.amberAccent.shade700, fontSize: 15, fontFamily: GoogleFonts.poppins().fontFamily))
                  ],
                ),
                ...
                const SizedBox(height: 20,),
                Text(formatToCustomFormat(bookingDetails[0]['date'].toString()), style: const TextStyle(
                          fontWeight: FontWeight.bold, fontSize: 16,
                        ),),
                const SizedBox(height: 10,),
                Text("${convert24HourTo12Hour(bookingDetails[0]['timeslot'])} - ${addOneHourToCurrentTime(convert24HourTo12Hour(bookingDetails[0]['timeslot']))}", style: const TextStyle(
                          fontWeight: FontWeight.bold, fontSize: 16,
                        ),),
                ...
                const SizedBox(height: 20,),
                 ...
                 Text(patientDetails[0]['name'], style: const TextStyle(
                          fontWeight: FontWeight.bold, fontSize: 16,
                        ),),
                const SizedBox(height: 10,),
                Text("Age : ${patientDetails[0]['age']}  Gender: ${patientDetails[0]['gender']}", style: const TextStyle(
                          fontWeight: FontWeight.bold, fontSize: 16,
                      ),),
        ...
        children: [
          Padding(
            ...
            child: ElevatedButton(
              onPressed: () {
                showAlert(context, 'Are you sure you want to cancel this booking?');
              },
              style: ButtonStyle(
                backgroundColor: const MaterialStatePropertyAll(Color.fromARGB(255, 211, 78, 78)),
                fixedSize: MaterialStateProperty.all(
                  const Size(150, 40), 
                ),
                  ),
              child: Text('Cancel Booking',style: TextStyle(fontFamily: GoogleFonts.poppins().fontFamily,
