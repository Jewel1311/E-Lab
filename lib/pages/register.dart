...
class _RegisterState extends State<Register> {
  ...
  Future registerUser() async {
    ...
    try {
      ...
      final profile =
          await supabase.from('profile').upsert([profileData]).select();

      await OneSignal.shared.getDeviceState().then((deviceState) {
        userId =
            deviceState!.userId.toString(); // Use this ID to identify the user
      });
      await supabase
          .from('profile')
          .upsert({'id': profile[0]['id'], 'onesignaluserid': userId});
      ...
      if (supabase.auth.currentUser != null) {
        ...
        final supabaseadmin =
            SupabaseClient(dotenv.env['URL']!, dotenv.env['SERVICE_KEY']!);
  ...
  //loading view
  Center loadingView() => const Center(
